...
int main(int argc, char **argv) {
  char t[] = "/tmp/XXXXXX";
  char uri[1024 + 1], buf[4096], buf2[2048], *bp;
  int i, j, siz, sfd = 0, hp, KB = 1024; mkstemp(t);
  FILE *his, *tmp;
  struct hostent *he;
  ...
  if (!(ctx = SSL_CTX_new(TLS_client_method())))
    errx(1, "SSL_CTX_new");
  if (!(his = fopen(".gmi100", "a+b")))
    err(1, "fopen(.gmi100)");
  if (!(tmp = fopen(t, "w+b")))
    err(1, "fopen(%s)", t);
  fseek(his, 0, SEEK_END), hp = ftell(his) - 1;
start:
  fprintf(stderr, "gmi100> "); /* Main loop */
  if (!fgets(buf, KB, stdin))
    return 0;
  if (*buf == '!') { /* Cmd */
    buf[strlen(buf) - 1] = 0;
  sys:
    sprintf(buf2, "%.256s %s", buf + 1, t);
    system(buf2);
    goto start;
  } else if (buf[1] == '\n')
    ...
    case 'q':
      return 0;
    case '?':
      sprintf(uri, "geminispace.info/search");
      goto query;
    case 'r':
      strcpy(buf, uri);
      goto uri; /* Refresh */
    case 'c':
      ...
      goto start;
    case 'u':
      sprintf(buf, "%.1021s../", uri);
      goto uri;
    case 'b':
      while (!fseek(his, --hp, 0) && hp && fgetc(his) != '\n')
        ;
      fgets(buf, KB, his);
      goto uri;
    }
  ...
  if ((i = atoi(buf)) > 0 && (siz = sprintf(buf, "[%d]\t=> ", i))) { /* Nav */
    ...
    if (strstr(bp, "//"))
      ...
      uri[strcspn(uri, "/\n\0")] = 0;
    else if (!strncmp(&bp[i], "../", 2))
      ; /* Keep whole uri */
    else if (!strchr(uri, '/'))
      strcat(uri, "/");
    else
      for (j = strlen(uri); j && uri[--j] != '/'; uri[j] = 0)
        ;
    sprintf(buf, "%s%s", uri, bp);
  } /* Typed URL */
uri:
  i = strstr(buf, "//") ? (strncmp(buf, "gemini:", 7) ? 2 : 9) : 0;
  ...
  if ((ssl = SSL_new(ctx)) == 0)
    WARN("SSL_new");
  if (!SSL_set_tlsext_host_name(ssl, buf))
    WARN("SSL_set_tlsext");
  if (!SSL_set_fd(ssl, sfd))
    WARN("SSL_set_fd");
  if (SSL_connect(ssl) < 1)
    WARN("SSL_connect");
  if (SSL_write(ssl, buf2, siz) < 1)
    WARN("SSL_write");
  ...
  query:
    ...
    goto uri;
  } else if (bp[0] == '3' && strcpy(buf, bp + 3))
    goto uri;                /* Redirect */
  fprintf(his, "%s\n", uri); /* Save URI in history */
  if (!(tmp = freopen(t, "w+b", tmp)))
    err(1, "freopen(%s)", t);
nodesc:
  if ((j = !strncmp(bp + 3, "text/", 5)))
    while (SSL_peek(ssl, bp, 2)) {
      if (!strncmp(bp, "=>", 2) && SSL_read(ssl, bp, 2)) {
        fprintf(tmp, "[%d]\t=> ", ++i); /* It's-a Mee, URIoo! */
        while (SSL_peek(ssl, bp, 1) && *bp <= ' ')
          SSL_read(ssl, bp, 1);
    ...
    while ((siz = SSL_read(ssl, bp, KB)))
      fwrite(bp, 1, siz, tmp);
  if (fflush(tmp))
    err(1, "fflush(tmp)");
  if (j && sprintf(buf, "?%.256s", argc > 1 ? argv[1] : "less -XI"))
    goto sys;
  goto start;
  return 0;
}